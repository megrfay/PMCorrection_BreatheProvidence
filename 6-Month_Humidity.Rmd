...
All_Sensors_Raw <- read.csv("C:/Users/mrfay/OneDrive - University of Vermont/Desktop/BPVD/PM/data/6month_pm25_raw.csv")

# convert date
All_Sensors_Raw$date <- as_datetime(All_Sensors_Raw$date, format =  "%Y-%m-%d %H:%M:%S")
#View(All_Sensors_Raw)
```

```{r take hourly averages of sensor data}
#Generate Hourly averages for pm2_5, temp, rh of SENSOR 1

MJF_hourly <-  All_Sensors_Raw %>%
  filter(Sensor == "Myron J. Francis Elementary") %>%
  mutate(date_hourly = floor_date(date, unit = "hour"))

MJF_pm2_5_hourly <- MJF_hourly %>%
  group_by(date_hourly) %>%
  summarise_at(vars(pm2_5), list(pm2_5 = mean))
...
MJF_hourly <- merge(MJF_pm2_5_hourly, MJF_rh_hourly, by = "date_hourly")
```
...
# create an hourly column in All_Sensors_Raw
All_Sensors_Raw <-  All_Sensors_Raw %>%
  mutate(date_hourly = floor_date(date, unit = "hour"))

# merge with RIDEM data
sensor_fit_merge <- merge(DEM_sensor, All_Sensors_Raw, by= "date_hourly")
...
sensor_fit_merge$pm2_5_dry <- (sensor_fit_merge$pm2_5)*(sensor_fit_merge$m/(1+sensor_fit_merge$k/((100/sensor_fit_merge$rh)-1)))
#View(sensor_fit_merge)
```

```{r save output}
# subset variables
PVD_PM_Cal <- subset(sensor_fit_merge, select = c(lat, lon, date.y, date_hourly, season, m, k, Sensor, code, rh, pm2_5, pm2_5_dry, PM2_5))
colnames(PVD_PM_Cal) <- c("lat", "lon", "date", "date_hourly", "season", "m", "k", "Sensor", "code", "rh", "pm2_5", "pm2_5_dry", "PM2_5_RIDEM")
#View(PVD_PM_Cal)
...
write.csv(PVD_PM_Cal_df, "C:/Users/mrfay/OneDrive - University of Vermont/Desktop/BPVD/PM/data/6month_pm25_humidity.csv")
```
